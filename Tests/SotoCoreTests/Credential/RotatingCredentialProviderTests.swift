...
class RotatingCredentialProviderTests: XCTestCase {
    ...
    func testGetCredentialAndReuseIfStillValid() {
        ...
        let context = CredentialProviderFactory.InitContext(httpClient: httpClient, eventLoop: loop, context: TestEnvironment.context)
        ...
        XCTAssertNoThrow(returned = try provider.getCredential(on: loop, context: TestEnvironment.context).wait())
